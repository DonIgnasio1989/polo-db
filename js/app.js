    ...
    async function lAll() {
      ...
      // 2. Подмешиваем с сервера — новые позиции по OEM+name
      try {
        const r = await fetch('data/parts.json', { cache: 'no-store' }).catch(() => null);
        // (data/parts.json больше нет — но не страшно, просто пропустит)
      } catch (e) { }
      ...
      let serverParts = [], anyOk = false;
      for (const f of files) {
        try {
          const r = await fetch(f, { cache: 'no-store' });
          if (!r.ok) { console.warn('Не найден:', f); continue; }
          const arr = await r.json();
          if (Array.isArray(arr) && arr.length) { serverParts = serverParts.concat(arr); anyOk = true; console.log('OK:', f, arr.length); }
        } catch (e) { console.warn('Ошибка ' + f, e); }
      }

      // 3. Мержим (не дублируем!)
      if (anyOk) {
        const seen = new Set(D.map(x => (x.oem || '') + '||' + (x.name || '')));
        let add = 0;
        for (const raw of serverParts) {
          ...
          if (!seen.has(k)) { D.push(np); seen.add(k); add++; }
        }
        console.log('С сервера подмешано:', add, '| Итого:', D.length);
        if (add && useIDB) try { await iMany(SP, D); } catch (e) { }
        else if (add && !useIDB) sD();
      }

      // 4. Категории из JSON
      try {
        const rs = await fetch('data/categories.json', { cache: 'no-store' }).catch(() => null);
        if (rs && rs.ok) {
          const cats = await rs.json();
      ...
      // 5. Секции (моменты, диагностика)
      try {
        const rsec = await fetch('data/sections.json', { cache: 'no-store' });
        if (rsec.ok) { const secs = await rsec.json(); if (secs && typeof secs === 'object') { CUSTOM.sections = Object.assign({}, secs); sMeta(); } }
      } catch (e) { }
    ...
    let tt = null;
    function toast(m, k) { const kk = k === 'success' ? 's' : k === 'danger' ? 'd' : (k || ''); const e = $('tt'); e.textContent = m; e.className = 'tst show' + (kk ? ' ' + kk : ''); clearTimeout(tt); tt = setTimeout(() => e.classList.remove('show'), 2400); }
    ...
    async function rD() {
      if (!confirm('Полный сброс БД: удалить все изменения, мета и загрузить заново?')) return;
      try {
        if (useIDB) await iClr(SP);
        localStorage.removeItem(PK);
        resetMeta();
        D = DB.map(nP).filter(Boolean);
        if (useIDB) await iMany(SP, D);
        else sD();
        rSB(); rC(); toast('БД пересобрана: ' + D.length + ' поз.', 'success');
